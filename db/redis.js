...
if (isRedisConnectionRequired) {
    ...
    if (process.env.AIEZE_COMMON_REDIS_USER && process.env.AIEZE_COMMON_REDIS_PASSWORD) {
        baseConnectionURI += `${process.env.AIEZE_COMMON_REDIS_USER}:${process.env.AIEZE_COMMON_REDIS_PASSWORD}`
    }
    
    let client = redis.createClient({ url: `${baseConnectionURI}@${process.env.AIEZE_COMMON_REDIS_HOST}:${process.env.AIEZE_COMMON_REDIS_PORT}` });
    
    let clientReader;
    
    if (process.env.AIEZE_COMMON_REDIS_HOST_READER) {
        clientReader = redis.createClient({ url: `${baseConnectionURI}@${process.env.AIEZE_COMMON_REDIS_HOST_READER}:${process.env.AIEZE_COMMON_REDIS_PORT}` });
    } else {
        logger.info("Redis replica host not found. falling back to the primary redis host");
        clientReader = redis.createClient({ url: `${baseConnectionURI}@${process.env.AIEZE_COMMON_REDIS_HOST}:${process.env.AIEZE_COMMON_REDIS_PORT}` });
    }
